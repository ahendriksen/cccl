...
inline barrier<thread_scope_block>::arrival_token arrive_tx(
...
> {
    ...
    static async_contract_fulfillment __synchronize(
        _Group && __g, __arch::__cuda<90>, __alignment<16>,
        barrier<_Sco> & __b, _CUDA_VSTD::size_t __size, async_contract_fulfillment __acf
    ) {
        if (__acf == async_contract_fulfillment::async && __g.thread_rank() == 0) {
    ...
    _LIBCUDACXX_DEVICE
    static async_contract_fulfillment __synchronize(
        _Group && __g, __arch::__cuda<90>, __alignment<_AlignmentV>,
        ...
        return __synchronize(__g, __arch::__cuda<80>{}, _Alignment{}, __b, __size, __acf);
    }
